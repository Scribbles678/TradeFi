...
const chartData = ref<WinRateDataPoint[]>([])
...
// Load win rate data by strategy
async function loadWinRateData() {
  ...
  try {
    ...
    const { data: trades, error } = await supabase
      .from('trades')
      .select('strategy_id, exit_time, is_winner')
      .gte('exit_time', thirtyDaysAgo.toISOString())
      .order('exit_time', { ascending: true })

    if (error) {
      console.error('Error loading win rate data:', error)
      return
    }

    // Process data into daily aggregates per strategy
    const dailyData: Record<string, Record<string, { trades: number; wins: number }>> = {}
    ...
    trades?.forEach(trade => {
      ...
      if (!dailyData[date][trade.strategy_id]) {
        dailyData[date][trade.strategy_id] = { trades: 0, wins: 0 }
      }
    ...
    const dataPoints: WinRateDataPoint[] = []
    const sortedDates = Object.keys(dailyData).sort()

    sortedDates.forEach(date => {
      const point: WinRateDataPoint = { date }
