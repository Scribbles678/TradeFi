...
-- on the Performance page. Replace YOUR_USER_ID with your actual user UUID.
--
-- Usage:
-- 1. Replace YOUR_USER_ID below with your actual user UUID
-- 2. Run this script in your Supabase SQL editor
-- 3. Refresh your Performance page to see the charts populated
-- ============================================================================

-- IMPORTANT: Replace this with your actual user UUID
-- You can find it in Supabase: Authentication > Users > Copy UUID
\set user_id 'e0470a70-f1f7-46bd-933f-b34afbcdb940'

-- ============================================================================
-- STEP 1: Create Active Strategies
-- ============================================================================
-- These strategies will appear in the Strategy Performance, Win Rate, and Signals charts
...
-- You can run this separately to see the IDs:
-- SELECT id, name FROM strategies WHERE status = 'active';

-- ============================================================================
-- STEP 2: Create Trades Spread Over Last 30 Days
-- ============================================================================
-- These trades are distributed across the last 30 days to populate:
-- - Strategy Performance Chart (P&L and Win Rate)
-- - Win Rate by Strategy Chart
-- - Signals by Strategy Chart (using entry_time)
-- - Asset Class Performance Chart

-- Strategy IDs will be referenced by name (you may need to adjust if IDs differ)
-- For now, we'll use a subquery to get strategy IDs dynamically

-- DAY 1-5: Momentum Breakout Strategy (Crypto)
...
WHERE EXISTS (SELECT 1 FROM strategies WHERE name = 'Momentum Breakout');
...
-- DAY 6-10: Forex Scalper Strategy
INSERT INTO trades (user_id, symbol, side, asset_class, exchange, entry_price, entry_time, exit_price, exit_time, quantity, position_size_usd, pnl_usd, pnl_percent, is_winner, exit_reason, strategy_id)
SELECT 
  :'user_id'::uuid,
...
WHERE EXISTS (SELECT 1 FROM strategies WHERE name = 'Forex Scalper');
...
-- DAY 11-15: Options Premium Strategy
INSERT INTO trades (user_id, symbol, side, asset_class, exchange, entry_price, entry_time, exit_price, exit_time, quantity, position_size_usd, pnl_usd, pnl_percent, is_winner, exit_reason, strategy_id)
SELECT 
  :'user_id'::uuid,
...
WHERE EXISTS (SELECT 1 FROM strategies WHERE name = 'Options Premium');
...
-- DAY 16-20: Trend Follower Strategy (Stocks)
INSERT INTO trades (user_id, symbol, side, asset_class, exchange, entry_price, entry_time, exit_price, exit_time, quantity, position_size_usd, pnl_usd, pnl_percent, is_winner, exit_reason, strategy_id)
SELECT 
  :'user_id'::uuid,
...
WHERE EXISTS (SELECT 1 FROM strategies WHERE name = 'Trend Follower');
...
-- DAY 21-25: Mixed strategies for variety
INSERT INTO trades (user_id, symbol, side, asset_class, exchange, entry_price, entry_time, exit_price, exit_time, quantity, position_size_usd, pnl_usd, pnl_percent, is_winner, exit_reason, strategy_id)
SELECT 
  :'user_id'::uuid,
  'BTCUSDT', 'BUY', 'crypto', 'aster',
...
SELECT id, name, status, asset_class FROM strategies WHERE status = 'active';
...
-- ============================================================================
-- DELETE FROM trades WHERE user_id = :'user_id'::uuid AND entry_time >= NOW() - INTERVAL '30 days';
